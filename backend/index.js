...
function buscarEstimativas(tipoAcao, tribunal, instancia) {
  ...
  return estimativasData.filter((est) => {
    return Object.keys(filtros).every(
      (key) =>
        est[key] && est[key].toLowerCase().includes(filtros[key].toLowerCase()),
    );
  });
}
...
app.get("/api/tribunais", (req, res) => {
  try {
    ...
    if (estado) {
      tribunaisFiltrados = tribunaisFiltrados.filter(
  ...
  try {
    ...
    if (estimativas.length === 0) {
      return res.status(404).json({
        ...
        sugestao:
          "Tente com critérios mais amplos ou consulte um advogado especializado",
      });
    }
    ...
    // Log da consulta
    console.log(
      `Estimativa solicitada: ${tipoAcao} - ${tribunal} | Resultado: ${tempoMedio?.texto}`,
    );
    ...
    res.json({
      estimativa: {
        ...
        tempo: tempoMedio,
        confiabilidade:
          estimativas.length >= 3
            ? "alta"
            : estimativas.length >= 2
              ? "media"
              : "baixa",
...
app.get("/api/estimativas", (req, res) => {
  try {
    ...
    if (tipo) {
      estimativas = estimativas.filter((est) =>
        est.tipo_acao?.toLowerCase().includes(tipo.toLowerCase()),
      );
    }

    if (tribunal) {
      estimativas = estimativas.filter((est) =>
        est.tribunal?.toLowerCase().includes(tribunal.toLowerCase()),
      );
    }

    if (instancia) {
      estimativas = estimativas.filter(
        (est) => est.instancia?.toLowerCase() === instancia.toLowerCase(),
      );
    }
